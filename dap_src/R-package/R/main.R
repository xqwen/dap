...
#'
#' @usage
#' dap(file, ens=1, pi1=-1, ld_control=0.25, msize=-1, converg_thresh=0.01, all=FALSE, size_limit=-1, thread=1)
#'
...
#' @export
dap <- function(file, ens=1, pi1=-1, ld_control=0.25, msize=-1, converg_thresh=0.01, all=FALSE, size_limit=-1, thread=1)
{
  if(is.null(file))
    stop("[Error] The option file is required!\n")
