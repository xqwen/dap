...
// for SNP effect size
vector<vector<double> > snp_beta_vec;
vector<vector<double> > snp_var_vec;
vector<vector<double> > snp_prob_vec;
...
int main (int argc, char **argv){ 
	...
	for(int i=0;i<genov.size();i++){
		...
		snp_beta_vec.push_back(vector<double>());
		snp_var_vec.push_back(vector<double>());
		snp_prob_vec.push_back(vector<double>());
	}
	...
	*/
	...
	// summarize snp effect size


	for (int i=0;i<snp_beta_vec.size();i++){
		double beta =0;
		double var =0;
		double prob = 0;
		for (int j=0;j<snp_beta_vec[i].size();j++){
			beta += snp_prob_vec[i][j]*snp_beta_vec[i][j];
			var  += snp_prob_vec[i][j]*(snp_var_vec[i][j]+pow(snp_beta_vec[i][j],2));
		        prob += snp_prob_vec[i][j];
		}
	...
	if(!er2_only){
		...
		// IMPORTANT: all fileds need to be separated by a single tab, otherwise make_GAMBIT_DB.R will not run.
		for(int i=0;i<coef_vec.size();i++){
			if(fabs(coef_vec[i]) >= weight_thresh)
				fprintf(out_fd, "%s\t%s\t%s\t%7.3e\t%7.3e\t%7.3e\n",tissue.c_str(), gene.c_str(), pars.geno_map[i].c_str(), coef_vec[i], beta_vec[i], se_vec[i]);
...
double run_regression(vector<int> &index_vec, double weight){
	...
	for(int i=0;i<p;i++){
		int k = index_vec[i];
		snp_beta_vec[k].push_back(gsl_matrix_get(bv,i+1,0));
		snp_prob_vec[k].push_back(weight);
		snp_var_vec[k].push_back(gsl_matrix_get(XtX_inv,i+1,i+1)*s2);
	}
