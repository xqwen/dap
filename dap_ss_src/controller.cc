...
void controller::init(char *grid_file, char *data_file, char *inc_file){  
 
  if(strlen(inc_file)==0){
    all_include = 1;
  }else{
    all_include = 0;
    set_inclusion_list(inc_file);
  }

  load_grid(grid_file);
  load_summary(data_file);
  ...
  sslr.init(yty,GtG,Gty,n);
  //sslr.set_prior_option(sslr_prior_option);
  sslr.set_effect_vec(phi2_vec);
...
void controller::set_inclusion_list(char *inc_file){

  ifstream gfile(inc_file);
  string line;
  istringstream ins;
...
void controller::load_summary(char *data_file){
  ifstream gfile(data_file);
  string line;
  istringstream ins;
  int count = 0;
  while(getline(gfile,line)){
    ...
    double sd_bhat;
    double freq;
    if(ins>>snp>>loc>>bhat>>sd_bhat>>freq){
      if(include_list[snp]==10||all_include){	
	//printf("%s \n", snp.c_str());
	geno_rmap[snp] = count;
	geno_map[count] = snp;
	count++;
	beta_vec.push_back(bhat);
	var_vec.push_back(sd_bhat*sd_bhat);
...
void controller::process_data(){
  ...
  for(int i=0;i<p;i++){
    gsl_matrix_set(GtG,i,i,1);
  }

  // setup yty if ! specify_yty
  if(!specify_yty){
    vector<double> yvec;
    for(int i=0;i<p;i++){
      double ytyi = ((n-2)*var_vec[i]+pow(beta_vec[i],2))*2*n*freq_vec[i]*(1-freq_vec[i]);
      yvec.push_back(ytyi);
    }
    std::sort(yvec.begin(),yvec.end());
    // take the median
    yty = yvec[int(yvec.size()/2)];
  }
  ...
  vector<double> gvec;
  for(int i=0;i<p;i++){
    double gtg = yty/((n-2)*var_vec[i]+pow(beta_vec[i],2));
    gvec.push_back(gtg);
    //printf("%d  %f\n",i+1, gtg);
  }
  ...
  for(int i=0;i<p;i++){
    double val = beta_vec[i]*gvec[i];
    gsl_matrix_set(Gty,i,0,val);
  }
  // setup GtG
  for(int i=0;i<p;i++){
    for(int j=0;j<p;j++){
      double val = gsl_matrix_get(GtG,i,j);
      val = val*sqrt(gvec[i]*gvec[j]);
...
void controller::set_prior(double pi1, double lambda){
...
void controller::run(){
  ...
  while(1){
    ...
    //fprintf(stderr,   "%7.3f   (%7.3f , %7.3f)    %f  %f  %f    \n", ncps, lb, rb,val, prev_val, size_select_thresh);
...
size_model controller::compute_post_model(int size, vector<int>& best_model){
  ...
  */
  ...
  for(int i=0;i<ms;i++){
    ...
    SSLR_SS local_sslr ;
    local_sslr.init(yty,GtG,Gty,n);
    local_sslr.set_sigma_option(sslr_sigma_option);
    local_sslr.set_effect_vec(my_phi2_vec);
