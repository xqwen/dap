...
double wgsl_optim_f(gsl_vector *v, void *params)
{
  double mLogLik=0;
  fix_parm_T *p = (fix_parm_T *)params;
  mLogLik = fLogitCat(v,p->X,p->nlev,p->Xc,p->y,p->lambdaL1,p->lambdaL2);
  return mLogLik; 
}
		 ...
		 ,double lambdaL2)
{
  ...
  for(iter=0;iter<100;iter++){ 
    wgsl_optim_hessian(beta,&p,myH); //Calculate Hessian
    wgsl_optim_df(beta,&p,myG);      //Calculate Gradient
