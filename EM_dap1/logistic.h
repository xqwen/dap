...
void logistic_pred(gsl_vector *beta  // Vector of parameters length = 1 + Sum_k(C_k - 1) + Kc
...
int logistic_fit(gsl_vector *beta  // Vector of parameters length = 1 + Sum_k(C_k - 1) + Kc
		 ,gsl_matrix_int *X  //Matrix Nobs x K 
		 ,gsl_vector_int *nlev // Vector with number categories
		 ,gsl_matrix *Xc   // continuous covariates  Matrix Nobs x Kc (NULL if not used)
